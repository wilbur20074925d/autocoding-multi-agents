...
- **HC1/HC2 human shorthand (parallel strands):** **`solution\development-*`** (ask, answer, agree, give, build on, …) → **`Cognitive.solution_development`**. **`concept\exploration-*`** (same sub-action names) → **`Cognitive.concept_exploration`**. **`planning-*`** (e.g. planning-give, planning-agree) → **`Metacognitive.planning`** — how to structure or sequence the work; **not** `Cognitive.solution_development` even if “question” or “bullet” appears. The **strand prefix** encodes the focus, not the sub-action verb alone. See **cloudbot/data/cognitive-tier2-hc-subactions.md**, **cloudbot/data/metacognitive-tier2-hc-subactions.md**, and **golden-labels.md**.

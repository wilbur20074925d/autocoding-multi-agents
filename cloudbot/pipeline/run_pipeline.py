...
def _utterance_looks_like_metacognitive_evaluating(t: str) -> bool:
    ...
    **Metacognitive.evaluating** (HC `evaluating-ask` / `evaluating-give`).

    **Not** **Cognitive.concept_exploration**, which asks what subject matter **means** in general
    (definitions, theory) — e.g. “What is Bloom’s taxonomy?”
    """
    tl = (t or "").strip().lower()
    if not tl:
        return False
    ...
    if re.search(
        r"(?i)^(what is|what are|what does|what are the|can you define|explain what|tell me what)\b",
        tl,
    ) and re.search(r"(?i)\b(bloom|taxonomy|concept|metacognitive|theory|term)\b", tl):
        return False
    if re.search(r"(?i)\bwhat (is|are)\b[^?.]{0,160}\b(bloom|taxonomy)\b", tl):
        return False
    if re.search(r"(?i)\bwhat does\b[^?.]{0,120}\b(mean|metacognitive)\b", tl):
        return False
    # Quality / adequacy assessment (evaluating-ask / evaluating-give)
    if re.search(r"(?i)\bdo you think\b", tl) and re.search(
        r"(?i)\b(make sense|enough|adequate|detail|quality|correct|strong|weak|good enough|ok|gpt)\b",
        tl,
    ):
        return True
    if re.search(r"(?i)\bdoes .{0,64} make sense\b", tl):
        return True
    if re.search(
        r"(?i)\bis (our|the|this|that|your) .{0,64}(ok|okay|enough|good|strong|correct|weak|clear|adequate|sufficient)\b",
        tl,
    ):
        return True
    if re.search(r"(?i)\bis (this|that|it) (good|ok|right|strong) enough\b", tl):
        return True
    if re.search(r"(?i)\benough detail\b", tl):
        return True
    if re.search(r"(?i)\black(s|ing)? (enough )?detail\b", tl):
        return True
    if re.search(r"(?i)\b(good enough|strong enough|weak enough)\b", tl):
        return True
    if re.search(r"(?i)\bmeet(s)? (the )?(criteria|requirements)\b", tl):
        return True
    if re.search(r"(?i)\bgpt (results|output|answer|response)\b", tl):
        return True
    if re.search(r"(?i)\b(quality of|evaluate the|assess the|assess our)\b", tl):
        return True
    if re.search(
        r"(?i)\b(is our|is the|are we).{0,96}(solution|explanation|answer|output).{0,48}(correct|ok|strong|good|enough)\b",
        tl,
    ):
        return True
    if re.search(r"(?i)\b(gpt|chatgpt|model output)\b", tl) and re.search(
        r"(?i)\b(lack|detail|enough|adequate|weak|strong|good|bad)\b",
        tl,
    ):
        return True
    return False
...
def _utterance_looks_like_taxonomy_concept_exploration_talk(t: str) -> bool:
    ...
    **Not** **Metacognitive.evaluating** (judging GPT/output quality — use
    `_utterance_looks_like_metacognitive_evaluating` instead).
    """
    tl = (t or "").strip().lower()
    if _utterance_looks_like_metacognitive_evaluating(t):
        return False
...
def _apply_metacognitive_evaluating_bias(raw: dict[str, float], t: str) -> None:
    """Boost Metacognitive.evaluating over CE when the line judges output quality, not definitions."""
    if not _utterance_looks_like_metacognitive_evaluating(t):
        return
    raw["Metacognitive.evaluating"] += 3.35
    raw["Cognitive.concept_exploration"] = max(
        0.0,
        raw.get("Cognitive.concept_exploration", 0.0) - 3.25,
    )
    raw["Cognitive.solution_development"] = max(
        0.0,
...
def _semantic_proxy_scores(
    ...
    if "?" in t:
        ...
        if not _utterance_looks_like_bloom_task_solution_talk(t):
            # Evaluating-ask questions judge adequacy — not definitional CE
            if not _utterance_looks_like_metacognitive_evaluating(t):
